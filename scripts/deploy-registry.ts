...
async function main() {
  ...
  const tealPath = path.join(__dirname, '..', 'contracts', 'registry.teal');
  ...
  // Deploy application
  console.log('Deploying Delegation Registry...');
  ...
  const appCreateTxn = algosdk.makeApplicationCreateTxnFromObject({
    ...
    numGlobalInts: 1,        // total_delegations
  ...
  console.log(`\nAdd to .env:`);
  console.log(`  REGISTRY_APP_ID=${appId}`);

  // Fund the contract account with minimum balance
